...
-- Throttled logging (every couple seconds)
local function log_ads_scale_throttled(s, weapon_extension)
	local now = nil
	...
	mod._last_ads_scale_log_t = mod._last_ads_scale_log_t or 0
	if (now - mod._last_ads_scale_log_t) < 2.0 then
		return
	end
	mod._last_ads_scale_log_t = now

	local wt = weapon_extension and weapon_extension:weapon_template()
	local wname = (wt and wt.name) or "unknown_weapon"
	mod:echo(string.format("[weapon_fov] ADS scale s=%.3f (weapon=%s)", s, wname))
end
...
local function ads_recoil_scale_from_template_and_slider(weapon_extension)
	-- Current ADS viewmodel FOV (degrees) from your mod (runtime table preferred, settings as fallback)
	...
	-- Baseline ADS viewmodel FOV (degrees) from weapon template (prefer orig_* if present)
	...
	-- Safety clamp (won't affect normal ranges)
...
function _update_aim_offset(dt, t, first_person_unit, unit_data_extension, weapon_extension, alternate_fire_component, lerp_values)
	...
	if visual_recoil_settings then
		...
		-- Lerp RAW recoil (unscaled) and store RAW in lerp_values
		...
		-- Apply dynamic scaling ONLY while ADS is active, and log s every couple seconds
		if alternate_fire_component.is_active then
			local ads_scale = ads_recoil_scale_from_template_and_slider(weapon_extension)
			log_ads_scale_throttled(ads_scale, weapon_extension)

			recoil_pitch_lerped = recoil_pitch_lerped * ads_scale
			recoil_yaw_lerped   = recoil_yaw_lerped   * ads_scale
		end
	...
	if local_player then
		...
		if alternate_fire_component.is_active then
			...
			if vertical_fov then
				...
				if weapon_template.alternate_fire_settings.camera.orig_vertical_fov then
		...
		-- Projection-correct scaling (better than base_tweak_fov/current_fov for high FOV)
